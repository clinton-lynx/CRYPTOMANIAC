  ...
  const NEWS_API_KEY: any = process.env.NEWS_API_KEY
    ...
    const data = {
        labels: chartData.map(
            (value : any) => moment(value.x).format('MMM DD')
        ),
        datasets: [
            {
                label: 'price to usd',
    ...
    const fetchData = async () => {
        try {
            const response = await axios(`https://api.coingecko.com/api/v3/coins/${coin}`);
    ...
    const fetchMArketChart = async () => {
        try {
            const response = await axios(`https://api.coingecko.com/api/v3/coins/${coin}/market_chart?vs_currency=usd&days=7`);
            console.log(response);
            const data = response.data;
            console.log(data);
            const dataObj = data.prices ?. map((value : any) => ({x: value[0], y: value[1].toFixed(2)}))
            console.log(dataObj);
                                ...
                                <div className="coin-chart-wrapper">
                                    <div className="coin-price-wrapper">
                                        <h2 className="coin-price">${
                                            millify(coinDetailsDev.market_data ?. current_price.usd, {precision: 2})
                                        }</h2>
                                        <span className="coin-price-per">
                                            {
                                            millify(coinDetailsDev.market_data ?. price_change_percentage_24h, {precision: 2})
                                        }%
                    ...
                    <div className={`${theme} coin__bottom-cointainer`}>
                        ...
                        loading ? (
                            ...
                            <section className="about-coin-container">
                                ...
                                <h3 className="coin__title">what is {
                                    coinDetails.name
                                }</h3>
                            ...
                            <div className="coin-info-wrapper">
                                <section className="coin__info">
                                    ...
                                    <ul className="coin__info-list">
                                        <li className="coin__info-list-item">
                                            <span className="coin__info-list-item-left">
                                                price
                                            </span>
                                            <span className="coin__info-list-item-right">${
                                                coinDetails.market_data ?. current_price.usd
                                            }</span>
                                        </li>
                                        <li className="coin__info-list-item">
                                            <span className="coin__info-list-item-left">24H Low / 24H High</span>
                                            <span className="coin__info-list-item-right">${
                                                millify(coinDetails.market_data ?. low_24h ?. usd)
                                            }
                                                / ${
                                                millify(coinDetails.market_data ?. high_24h ?. usd)
                                            }</span>
                                        </li>
                                        <li className="coin__info-list-item">
                                            <span className="coin__info-list-item-left">price change % 24H</span>
                                            <span className="coin__info-list-item-right">
                                                {
                                                millify(coinDetails.market_data ?. price_change_percentage_24h, {precision: 3})
                                            }%
                                            </span>
                                        </li>
                                        <li className="coin__info-list-item">
                                            <span className="coin__info-list-item-left">Trading volume</span>
                                            <span className="coin__info-list-item-right">${
                                                millify(coinDetails.market_data ?. total_volume ?. usd, {precision: 2})
                                            }</span>
                                        </li>
                                        <li className="coin__info-list-item">
                                            <span className="coin__info-list-item-left">market cap rank</span>
                                            <span className="coin__info-list-item-right">#{
                                                coinDetails.market_data ?. market_cap_rank
                                            }</span>
                                        </li>
                                        <li className="coin__info-list-item">
                                            <span className="coin__info-list-item-left">market cap</span>
                                            <span className="coin__info-list-item-right"></span>${
                                            millify(coinDetails.market_data ?. market_cap ?. usd)
                                        }</li>
                                ...
                                <section className="coin__info">
                                    ...
                                    <ul className="coin__info-list">
                                        <li className="coin__info-list-item">
                                            ...
                                            <span className="coin__info-list-item-right">
                                                <a target="_blank"
                                                    ...
                                                    className="coin__info-list-item-right-crumb">
                                                    {
                                                    coinDetails.name
                                                }.org</a>
                                            </span>
                                        ...
                                        <li className="coin__info-list-item">
                                            <span className="coin__info-list-item-left">blockchain sites</span>
                                            <span className="coin__info-list-item-right">
                                                <a target="_blank"
                                                    href={
                                                        coinDetails.links ?. blockchain_site[0]
                                                    }
                                                    className="coin__info-list-item-right-crumb">mintscan</a>
                                                <a target="_blank"
                                                    href={
                                                        coinDetails.links ?. blockchain_site[2]
                                                    }
                                                    className="coin__info-list-item-right-crumb">etherscan</a>
                                            </span>
                                        </li>
                                        <li className="coin__info-list-item">
                                            <span className="coin__info-list-item-left">communities</span>
                                            <span className="coin__info-list-item-right">
                                                <a target="_blank"
                                                    href={
                                                        coinDetails.links ?. chat_url[0]
                                                    }
                                                    className="coin__info-list-item-right-crumb">discord</a>
                                                <span className="coin__info-list-item-right-crumb">github</span>
                                                <span className="coin__info-list-item-right-crumb">reddit</span>
                                            </span>
                                        </li>
                                        <li className="coin__info-list-item">
                                            <span className="coin__info-list-item-left">source code</span>
                                            <span className="coin__info-list-item-right">
                                                <a target="_blank"
                                                    href={
                                                        coinDetails.links ?. repos_url ?. bitbucket[0]
                                                    }
                                                    className="coin__info-list-item-right-crumb">github</a>
                                                <a target="_blank"
                                                    href={
                                        ...
                                        <li className="coin__info-list-item">
                                            <span className="coin__info-list-item-left">tags</span>
                                            <span className="coin__info-list-item-right">
                                                <span className="coin__info-list-item-right-crumb">cryptocurrency</span>
                                            </span>
                                        </li>
                                    </ul>
                                </section>
